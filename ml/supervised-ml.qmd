...
# Supervised Learning {#sec-supervised-ml}
...
In supervised learning, the foundation is the *training set*, a collection of $N$ input–output pairs:

$$
\mathcal{D} = \{(\mathbf{x}_i, y_i)\}_{i=1}^N
$$

where $\mathbf{x}_i$ are the inputs (sometimes called features or feature vectors) and $y_i$ are the outputs (or labels)
are labels (outputs). The key goal of supervised learning is to learn a mapping from inputs to outputs, $f: \mathbf{x} \rightarrow y$ which can be expressed mathematically as a function $f(\mathbf{x}_i) \approx y_i$.
...
The boosted decision tree (BDT) is a commonly used model for classification and regression problems. It is a type of ensemble learning method, which combines the predictions of multiple models to improve overall performance.
...
Most of the models we will consider in this unit will be neural networks, which are described later in @sec-nn.  They are composed of layers of interconnected nodes (neurons), where each connection has an associated weight that is adjusted during training to minimise the loss function.  Such models are particularly well-suited to learning complex, non-linear relationships between inputs and outputs. 
...
Within regression problems, two commonly used loss functions are Mean Squared Error and Mean Absolute Error. 
...
## Overfitting/Underfitting {#sec-overfitting}

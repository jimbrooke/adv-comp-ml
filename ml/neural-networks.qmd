...
![An example of an Artifical Neural Network](figures/nn-simple.png){#fig-nn-anatomy}
...
Having computed the output of the network for a given set of weights, and the loss, we now want to iteratively update the weights to minimise the loss.  This will require computing the gradient of the loss with respect to the weights.  Backpropagation is an efficient method for computing this gradient, and is discussed in more detail in @sec-backpropagation.

### Weight Optimisation {#sec-weights-update}

Many methods exist for updating the weights.  The most commonly used are based on gradient descent, described in @sec-minimisation, in which a new set of weights is computed by taking a step in the direction of the negative gradient :

$$
\theta_{new} = \theta_{old} - \eta \nabla L
$$

where $\theta$ refers to the full set of weights $\alpha$ and biases $\beta$, and the _learning rate_, $\eta$ is a parameter which controls the length of the step, ie. how far the new set of weights is from the previous set.

Many NN training methods rely on a version of gradient descent. These methods differ in the data samples used to compute the gradient, and how the learning rate is chosen.  The most straightforward version of gradient descent involves computing the gradient using the full training sample, and then updating the weights.  However, with large datasets this becomes prohibitively computationally expensive, and more efficient methods are required.

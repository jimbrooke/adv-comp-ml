...
### Weights Update {#sec-weights-update}

Many methods exist for updating the weights.  The most commonly used are based on gradient descent, which was described in @sec-optimisation.  In simple gradient descent, a new set of weights is computed by taking a step in the direction of the negative gradient :

$$
\theta_{new} = \theta_{old} - \kappa \nabla L
$$

where $\theta$ refers to the full set of weights $\alpha$ and biases $\beta$, and the _learning rate_, $\kappa$ is a parameter which controls the length of the step, ie. how far the new set of weights is from the previous set.

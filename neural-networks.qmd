...
This is a multi-variable activation function, typically used only in the final layer of an ANN.
...
In order to train a neural network, the gradient of the loss with respect to the weights must be computed. The standard method for achieving this is backpropagation.

We can write the network as a combination of matrix multiplication and function composition :

$$

$$

which allows the loss to be written as a function of the weights :

$$

$$

Note that, for training purposes, we treat the input values as constants. 

Provided the derivative of the activation functions $f$ can be computed, the derivative of the loss with respect to each weight can be computed using the chain rule.  However, this is computationally inefficient.  Backpropagation relies on the fact that a weight $w_l$ can only affect the loss via the next layer.  Thus, we can compute the gradient with respect to weights of the final layer, then the next-to-final layer, and so on.
